...
Mainwin::Mainwin() {
    ...
    menuitem_undo->signal_activate().connect(sigc::mem_fun(*this, 
        &Mainwin::on_undo_click));
    ...
    menuitem_dash_pattern->signal_activate().connect(sigc::mem_fun(*this,
        &Mainwin::on_dash_pattern_click));
    ...
    Gtk::ToolButton *undo_button = Gtk::manage(new Gtk::ToolButton(Gtk::Stock::UNDO));
    undo_button->set_tooltip_markup("Undo the last line(s) drawn");
    undo_button->signal_clicked().connect(sigc::mem_fun(*this, &Mainwin::on_undo_click));
    ...
    button_freehand->set_tooltip_markup("Freehand");
...
//
// The user wants to draw segments
//
...
//
// The user wants to draw contiguous segments
//
...
//
// The user wants to draw freehand
//
...
//
// The user wants to change the dash pattern of lines
//
void Mainwin::on_dash_pattern_click() {

    // Create a custom dialog for selecting the dash pattern
    Gtk::Dialog dialog{"Select Dash Pattern", *this};

    // The lower widget is a drop-down list of patterns
    // The patterns are actually defined as View::dashes
    // This list MUST match that vector!
    // A dynamically constructed list of pattern images 
    // would be better, given time. Definitely next release! :-D
